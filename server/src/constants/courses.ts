...
// Main Campus offerings listed on the official CPSU program-offerings pages.
// Keep the short code as the persisted value so elections and voter profiles
// compare consistently even when the display name changes.
...
export function isKnownCourse(value: unknown): boolean {
  return COURSE_CODES.includes(normalizeCourse(value));
}
